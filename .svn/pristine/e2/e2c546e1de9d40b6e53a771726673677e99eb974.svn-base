...
volatile const uint8_t bitmaps[6][10] EEMEM = {
	{0xc3,0xc3,0x00,0x18,0x18,0x81,0xff,0x7e,0x00,0x00}, // smiley 3 small
	{0x3c,0x42,0x81,0x81,0xc3,0x24,0xa5,0xe7,0x00,0x00}, // Omega small
	{0x00,0x04,0x06,0xff,0xff,0x06,0x04,0x00,0x00,0x00}, // Arrow small
	{0x81,0x42,0x24,0x18,0x18,0x24,0x42,0x81,0x00,0x00}, // X		small
	{0xBD,0xA1,0xA1,0xB9,0xA1,0xA1,0xA1,0x00,0x00,0x00}, // ifi	small
	{0xEF,0x48,0x4B,0x49,0x4F,0x00,0x00,0x00,0x00,0x00}  // TG	small
};

volatile const uint8_t bigbitmaps[2][10] EEMEM = {
	{0x00,0x00,0x01,0x03,0x03,0x03,0x01,0x00,0x02,0x03}, // Omega big
	{0x78,0x84,0x02,0x03,0x03,0x03,0x02,0x84,0x49,0xCF}	
};

/*
{0x00,0x00,0x02,0x03,0x03,0x03,0x02,0x00,0x01,0x03}, // Omega big
	{0x1E,0x42,0x40,0x30,0x30,0x30,0x40,0x42,0x92,0xF3}	
		{0x0F,0x0F,0x0F,0x0F,0x00,0x00,0x00,0x00,0x00,0x00},
	{0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0xFF,0xFF,0xFF}
		*/

const unsigned char paths[51] PROGMEM =
{0x99,0x98,0x97,0x96,0x95,0x94,0x93,0x92,0x91,0x90,0x80,0x70,0x60,0x50,0x40,0x30,0x20,0x10,0x00,
0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x19,0x29,0x39,0x49,0x59,0x69,0x79,0x89,
0x04,0x03,0x12,0x21,0x30,0x40,0x51,0x62,0x73,0x74,0x65,0x56,0x47,0x37,0x26,0x15};

/*
const unsigned char paths[48] PROGMEM =
{0x07,0x06,0x05,0x04,0x03,0x02,0x01,0x00,0x10,0x20,0x30,0x40,0x50,0x60,0x70,		// circle
0x71,0x72,0x73,0x74,0x75,0x76,0x77,0x67,0x57,0x47,0x37,0x27,0x17,					// len 20(16)
0x04,0x03,0x12,0x21,0x30,0x40,0x51,0x62,0x73,0x74,0x65,0x56,0x47,0x37,0x26,0x15};	// offset 32(28)
*/
...
void font_getbitmap (char bitmap, uint16_t dst[10])
{
    ...
    for (i = 0; i < 10; i++)
		dst[i] = (uint8_t)eeprom_read_byte((uint8_t*)&bitmaps[(uint8_t)bitmap][(uint8_t)i]);
...
{
	uint16_t tmp1 = eeprom_read_byte((uint8_t*)&bigbitmaps[(uint8_t)bitmap][(uint8_t)x]);
	uint16_t tmp2 = eeprom_read_byte((uint8_t*)&bigbitmaps[(uint8_t)bitmap+1][(uint8_t)x]);
	uint16_t tmp = tmp2;
	tmp |=  (tmp1 << 8);
	return (tmp >> y) & 0x0001;
}